...
import org.cbioportal.legacy.service.importer.ImportService;
...
public class ImportController {
  ...
  @RequestMapping(
      value = "/api/logs/{logType}/{studyId}/{id}",
  ...
  public ResponseEntity<ImportLog> getLog(
    ...
    try (CloseableHttpResponse response = httpClient.execute(request)) {
      ...
      ImportLog importLog =
          mapper.readValue(EntityUtils.toString(response.getEntity()), ImportLog.class);
      return new ResponseEntity<>(
          importLog, status == null ? HttpStatus.INTERNAL_SERVER_ERROR : status);
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  @RequestMapping(
      value = "/api/logs/{logType}/{studyId}",
      method = RequestMethod.GET,
      produces = MediaType.APPLICATION_JSON_VALUE)
  @Operation(description = "Get the specified log file")
  @ApiResponse(
    ...
    try (CloseableHttpResponse response = httpClient.execute(request)) {
      ...
      List<ImportLog> importLogs =
          Arrays.asList(
              mapper.readValue(EntityUtils.toString(response.getEntity()), ImportLog[].class));
      return new ResponseEntity<>(
          importLogs, status == null ? HttpStatus.INTERNAL_SERVER_ERROR : status);
    } catch (Exception ex) {
      ex.printStackTrace();
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  @ApiResponse(
      ...
      description = "OK",
      content =
          @Content(array = @ArraySchema(schema = @Schema(implementation = ImportStudy.class))))
  public ResponseEntity<List<ImportStudy>> getAllImporterStudies(Authentication authentication) {
    ...
    try (CloseableHttpResponse response = httpClient.execute(request)) {
      HttpStatus status = HttpStatus.resolve(response.getStatusLine().getStatusCode());
      ObjectMapper mapper = new ObjectMapper();
      List<ImportStudy> studies =
          Arrays.asList(
              mapper.readValue(EntityUtils.toString(response.getEntity()), ImportStudy[].class));
      return new ResponseEntity<>(
          studies, status == null ? HttpStatus.INTERNAL_SERVER_ERROR : status);
    } catch (Exception ex) {
      ex.printStackTrace();
      return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  @RequestMapping(
      value = "/api/importer/{studyId}/import",
      method = RequestMethod.GET,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  public ResponseEntity<String> runTrialImport(
    ...
    HttpGet request =
        new HttpGet("http://importer:8080/importer/" + studyId + "/" + username + "/import");
    setUserIdHeader(authentication, request);
  ...
  @RequestMapping(
      value = "/api/importer/{studyId}/validate",
      method = RequestMethod.GET,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  public ResponseEntity<String> runTrialValidation(
    ...
    String username = getUserName(authentication);
    HttpGet request =
        new HttpGet("http://importer:8080/importer/" + studyId + "/" + username + "/validate");
    setUserIdHeader(authentication, request);
